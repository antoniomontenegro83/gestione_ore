...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    ...
    // Imposta le date predefinite al mese corrente
    impostaDateMeseCorrente();
...
/**
 * Imposta le date di default per il mese corrente
 */
function impostaDateMeseCorrente() {
  const oggi = new Date();
  ...
  if (dataInizioInput) dataInizioInput.value = primoDelMese.toISOString().split('T')[0];
  if (dataFineInput) dataFineInput.value = ultimoDelMese.toISOString().split('T')[0];
}
...
function setupEventListeners() {
  ...
  // Evento per il cambio delle date
  ...
  if (dataFine) {
    dataFine.addEventListener('change', function() {
      ...
      if (this.value && dataInizioValue) {
        generaReport();
      }
    });
  }
  
  // Evento per i pulsanti di esportazione
  const stampaReportBtn = document.getElementById('stampaReportBtn');
  if (stampaReportBtn) {
    stampaReportBtn.addEventListener('click', stampaReport);
  }
  ...
  const esportaPdfBtn = document.getElementById('esportaPdfBtn');
  if (esportaPdfBtn) {
    esportaPdfBtn.addEventListener('click', esportaPdf);
  }
}
...
function visualizzaReport(turni) {
  ...
  tableHTML += `
    ...
    <div class="mt-3 text-end">
      <button id="stampaReportBtn" class="btn btn-outline-secondary">
        <i class="bi bi-printer"></i> Stampa
      </button>
      <button id="esportaExcelBtn" class="btn btn-success ms-2">
        <i class="bi bi-file-excel"></i> Excel
      </button>
      <button id="esportaPdfBtn" class="btn btn-danger ms-2">
  ...
  // Riassegna gli event listener ai nuovi pulsanti
  const stampaReportBtn = document.getElementById('stampaReportBtn');
  if (stampaReportBtn) {
    stampaReportBtn.addEventListener('click', stampaReport);
  }
