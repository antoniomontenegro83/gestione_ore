...
export const TurniUI = {
  ...
  createTableRow(turno, isAdmin) {
    ...
    const sede = turno.sede || 'Non specificata';
    
    let azioni = '';
    if (isAdmin) {
      azioni = `
        <button class="btn btn-sm btn-warning me-1" onclick="window.apriModifica(${turno.id})">
          Modifica
        </button>
    ...
    row.innerHTML = `
      ...
      <td>${turno.cognome}</td>
      <td>${turno.nome}</td>
      <td><span class="badge bg-${this.getColorForSede(sede)}">${sede}</span></td>
      <td>${ingresso}</td>
      <td>${uscita}</td>
      <td>${azioni}</td>
    `;
  ...
  populateModifyModal(turno) {
    ...
    document.getElementById('modificaIngresso').value = `${turno.entry_date}T${turno.entry_time.substring(0, 5)}`;
    document.getElementById('modificaUscita').value = `${turno.exit_date}T${turno.exit_time.substring(0, 5)}`;
    
    // Imposta la sede nel select
    const sedeSelect = document.getElementById('modificaSede');
    if (sedeSelect) {
      sedeSelect.value = turno.sede || '';
    }
  },
