...
const Api = {
  ...
  /**
   * Pulisce la risposta JSON da eventuali commenti PHP o caratteri non validi
   * @param {string} responseText - Testo della risposta
   * @returns {string} - Risposta pulita
   */
  cleanResponse: function(responseText) {
    // Prima rimuovi eventuali BOM (Byte Order Mark)
    ...
    if (jsonObjStart > 0) {
      return responseText.substring(jsonObjStart);
    }
  ...
  get: async function(endpoint, params = {}) {
    try {
      ...
      try {
        ...
        console.error("Risposta pulita:", cleanedResponse);
        
        // Prova a identificare il problema
        if (cleanedResponse.length > 0) {
          console.error("Carattere alla posizione indicata dall'errore:", 
                       cleanedResponse.charCodeAt(jsonError.message.match(/position (\d+)/)?.[1] || 0));
        }
